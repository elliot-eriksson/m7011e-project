...
#############################################OWNER############################################################
    ...
    def test_transactions_budget_owner(self, mock_validate_token):
        ...
        session['user_id'] = self.owner.id
        session.save()
        print(f"/api/transactions/by-budget/{self.budget.slug}/")
        response = self.client.get(f"/api/transactions/by-budget/{self.budget.slug}/")
        print("heehe",response.json())
        self.assertEqual(response.status_code, 200)
    ...
    def test_transactions_user_owner(self, mock_validate_token):
        ...
        response = self.client.get(f"/api/transactions/by-user/{self.owner.slug}/")
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.get(f"/api/transactions/by-user/{self.admin.slug}/")
