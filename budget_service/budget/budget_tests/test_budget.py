...
class BudgetCaseTest(APITestCase):
    ...
    def setUp(self):
        ...
        BudgetAccess.objects.create(budget=self.budget, user=self.admin.id, accessLevel=BudgetRole.admin, accepted=True, slug="admin")
        BudgetAccess.objects.create(budget=self.budget, user=self.member.id, accessLevel=BudgetRole.member, accepted=True, slug="member")
    ...
    def test_owner_permissions(self, mock_validate_token):
        ...
        response = self.client.put(
            f"/api/budgets/{self.budget.slug}/",
            data=json.dumps({
        ...
        response = self.client.get(f"/api/budgets/{self.budget.slug}/")
        self.assertEqual(response.json(), expected_response)

        response = self.client.delete(f"/api/budgets/{self.budget.slug}/")
...
class BudgetListCreateTest(APITestCase):
    ...
    def test_create_budget(self, mock_validate_token):
        ...
        del response_data['slug'] # slug is generated randomly, so we can't predict it
    ...
    def test_list_budgets(self, mock_validate_token):
        ...
        BudgetAccess.objects.create(budget=self.budget, user=self.admin.id, accessLevel=BudgetRole.admin, accepted=True, slug="admin2")
